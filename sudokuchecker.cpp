...
void extract_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int side = std::sqrt( in.size() );
    int subsq_side = std::sqrt( side );
    for (int j = subs - subs%subsq_side ; j < subs - subs%subsq_side + subsq_side   ; j++){
        for (int i = subsq_side*(subs%subsq_side) ; i < subsq_side*(subs%subsq_side) + subsq_side   ; i++){
            out.push_back( in[ xytoi(j, i, side) ]  );
        }
    }
}
